...
public class CorfuServer {
    ...
    private static void printStartupMsg(Map<String, Object> opts) {
        ...
        if (opts.get("--rdma-port") != null) {
            println("RDMA on " + opts.get("--rdma-host") + ":" + Integer.parseInt(((String) opts.get("--rdma-port"))));
        } else {
            println("RDMA is not enabled, we fall back to ethernet");
        }
    }
}