...
public class LogUnitServer extends AbstractServer {
    public class RdmaRpcService extends RdmaRpcProtocol implements DaRPCService<RdmaRpcRequest, RdmaRpcResponse> {
        ...
        private void handleWrite(RdmaRpcRequest request) {
            LogData logData = new LogData(Unpooled.wrappedBuffer(request.getLogDataBuffer()));

            log.info("handleWrite: {}, at: {}, ids: {}",
        ...
        public void open(DaRPCServerEndpoint<RdmaRpcRequest, RdmaRpcResponse> endpoint) {
            log.info("new connection " + endpoint.getEndpointId() + ", cluster " + endpoint.clusterId());
        }
    
        @Override
        public void close(DaRPCServerEndpoint<RdmaRpcRequest, RdmaRpcResponse> endpoint) {
            log.info("disconnecting " + endpoint.getEndpointId() + ", cluster " + endpoint.clusterId());
        }
    }

    private String rdmaHost;
    ...
    private RdmaRpcService rpcService;

    void launchRdma() throws Exception {
        long[] clusterAffinities = new long[poolsize];
		for (int i = 0; i < poolsize; i++){
			long cpu = 1L << i;
			clusterAffinities[i] = cpu;
		}
        this.rpcService = new RdmaRpcService();
        DaRPCServerGroup<RdmaRpcRequest, RdmaRpcResponse> group = DaRPCServerGroup.createServerGroup(this.rpcService, clusterAffinities, -1, maxinline, polling, recvQueue, sendQueue, wqSize, 32); 
        RdmaServerEndpoint<DaRPCServerEndpoint<RdmaRpcRequest, RdmaRpcResponse>> serverEp = group.createServerEndpoint();

        InetSocketAddress address = new InetSocketAddress(rdmaHost, rdmaPort);
    ...
    public LogUnitServer(ServerContext serverContext, LogUnitServerInitializer serverInitializer) {
        ...
        this.rdmaHost = serverContext.getRdmaHost();
        this.rdmaPort = serverContext.getRdmaPort();
        ...
        if (this.rdmaPort == -1) {
            System.out.println("Rdma not available");
            return;
        }
    ...
    @RequestHandler(type = PayloadCase.RDMA_SERVER_REQUEST)
    private void handleRdmaServerRequest(RequestMsg req, ChannelHandlerContext ctx, IServerRouter router) {
        router.sendResponse(getResponseMsg(getHeaderMsg(req.getHeader()),
                getRdmaServerResponseMsg(this.rdmaHost, this.rdmaPort)), ctx);
    }
